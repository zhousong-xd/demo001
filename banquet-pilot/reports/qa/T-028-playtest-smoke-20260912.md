# T-028 候选 playtest 冒烟独立复核（质检·非真机）

> Issue: https://github.com/zhousong-xd/demo001/issues/34  
> 执行：宴席·质检 · role:BOT · work:qa  
> 作者工程冒烟（T-016…T-027）≠本环独立验收。  
> ACCEPT 由主导落盘；本文件只给候选 playtest 冒烟结论。
...
| Role | Value |
|------|-------|
| **Engineering tip under test** | `20600b969c8d48b4dbd6426e44ef01d52deed1ee` |
...
| **Script** | `banquet-pilot/candidates/playtest/smoke.mjs`（先审后跑） |
...
本环测的是 `candidates/playtest`，不是改正式包。锚点匹配，未 BLOCKED。
...
证据：`banquet-pilot/evidence/t028-playtest-smoke/`（command / stdout / stderr / full-log / dist-proof / NOTES）。
...
最低用例均 **PASS**；真机/多指/UI 记 **未测**（不记 PASS）。无 P0–P3。产品 dist 未改。

**VERDICT: PASS**（候选 playtest 冒烟视角。ACCEPT 由主导落盘。作者自测 ≠ 独立验收。）

---

## 7. 经验一行

候选 playtest 冒烟可在 node 内核层独立复验；正式包仍冻结，UI/真机另开环。
