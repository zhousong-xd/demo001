...
| Role | Value |
|------|-------|
| **Engineering tip under test** | `f645673f337bf7de3ad9f193c62e3ded5df7c628` |
...
| **Script** | `banquet-pilot/tests/evidence/scripts/t041-playtest-ui-critical-regression.mjs`（先审后复跑同一脚本） |
| **Script blob** | `29f0b490566766c700ad2a73e9a6c8bddd2735e7`（worktree == tip） |
...
| Command | `cd banquet-pilot && node --experimental-websocket tests/evidence/scripts/t041-playtest-ui-critical-regression.mjs` |
| Window (UTC) | 2026-09-12T08:45:08Z → 08:45:33Z |
| Exit code | `0` |
| Banner | place / undo / switch / swap / displace / calm miss / calm hit 均 `[PASS]`；真机/多指记未测 |
| 作者证据审读 | PASS（`evidence/t041-playtest-ui-critical-regression/`：脚本覆盖放撤切换顶铃；consolidates T-031/033/035；expected sha256 与冻结锚点一致） |
...
独立复跑同一脚本 + 抽检截图；不照搬作者 T-041 自测结论。
...
证据：`banquet-pilot/evidence/t042-playtest-critical-regression/`（NOTES / command / rerun-stdout / shots / dist-proof / script-summary）。
...
候选试玩页关键路径一键回归顺——放撤切换顶铃七步全绿；真机触屏仍未测，正式包仍冻结。
