/**
 * T-041 candidates playtest UI mouse-sim smoke (CDP Input.dispatchMouseEvent).
 * Re-run critical UI regression after T-039/T-040 tip (one command); same matrix as T-037.
 * 真机 / 多指: 未测 — CDP ≠ 真机.
 *
...
const EVID = path.join(ROOT, "evidence/t041-playtest-ui-critical-regression");
...
function buildChildEnv(extra = {}) {
  ...
  const env = {
    ...
    XDG_CONFIG_HOME: path.join(tmp, "banquet-t041-xdg-config"),
    XDG_CACHE_HOME: path.join(tmp, "banquet-t041-xdg-cache"),
    XDG_DATA_HOME: path.join(tmp, "banquet-t041-xdg-data"),
  };
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), "banquet-t041-chrome-"));
  ...
  try {
    ...
    // 1) place / undo / switch c01→c02
    await cdp.send("Page.navigate", { url: base + "?c=c01" });
    await sleep(800);
    await waitPlaytest(cdp);
    let s = await snap(cdp);
    if (s.fileId !== "c01") throw new Error("boot c01 fail: " + JSON.stringify(s));
    await screenshot(cdp, path.join(EVID, "shots/01_c01_boot.png"));
    await clickChar(cdp, "fox");
    await clickSeat(cdp, "A1");
    s = await snap(cdp);
    if (s.assignment?.fox !== "A1") throw new Error("place fail: " + JSON.stringify(s));
    await screenshot(cdp, path.join(EVID, "shots/02_c01_placed.png"));
    results.push({ step: "place_c01", pass: true });
    note("[PASS] place fox→A1");

    await clickUndo(cdp);
    s = await snap(cdp);
    if (Object.values(s.assignment || {}).filter(Boolean).length !== 0 || s.historyLen !== 0) {
      throw new Error("undo fail: " + JSON.stringify(s));
    }
    await screenshot(cdp, path.join(EVID, "shots/03_c01_undo.png"));
    results.push({ step: "undo_c01", pass: true });
    note("[PASS] undo empty");

    await switchCandidateMouse(cdp, "c02");
    await waitPlaytest(cdp);
    s = await snap(cdp);
    if (s.fileId !== "c02" || s.levelId !== "C02" || !String(s.url).includes("c=c02")) {
      throw new Error("switch c02 fail: " + JSON.stringify(s));
    }
    await screenshot(cdp, path.join(EVID, "shots/04_c02_switched.png"));
    results.push({ step: "switch_c02", pass: true });
    note("[PASS] switch c01→c02");

    // 2) swap on c02 (already there)
    await clickChar(cdp, "fox");
    await clickSeat(cdp, "A1");
    await clickChar(cdp, "rabbit");
    await clickSeat(cdp, "A2");
    await clickChar(cdp, "fox");
    await clickSeat(cdp, "A2");
    s = await snap(cdp);
    if (s.assignment?.fox !== "A2" || s.assignment?.rabbit !== "A1") {
      throw new Error("swap fail: " + JSON.stringify(s));
    }
    await screenshot(cdp, path.join(EVID, "shots/05_c02_swapped.png"));
    results.push({ step: "swap_c02", pass: true });
    note("[PASS] swap c02");
    ...
    await clickSeat(cdp, "A1");
    s = await snap(cdp);
    if (s.assignment?.rabbit !== "A1" || s.assignment?.fox !== null) {
      throw new Error("displace fail: " + JSON.stringify(s));
    }
    await screenshot(cdp, path.join(EVID, "shots/06_c01_displaced.png"));
    results.push({ step: "displace_c01", pass: true });
    note("[PASS] displace c01");

    // 4) calm bell c03
    await cdp.send("Page.navigate", { url: base + "?c=c03" });
    await sleep(800);
    await waitPlaytest(cdp);
    s = await snap(cdp);
    const stock0 = s.inventory?.calm_bell ?? 0;
    if (stock0 < 1) throw new Error("no bell stock: " + JSON.stringify(s));
    await clickBellButton(cdp);
    await clickChar(cdp, "fox");
    s = await snap(cdp);
    if ((s.inventory?.calm_bell ?? -1) !== stock0) throw new Error("miss consumed: " + JSON.stringify(s));
    await screenshot(cdp, path.join(EVID, "shots/07_c03_miss.png"));
    results.push({ step: "calm_miss_fox", pass: true });
    note("[PASS] calm miss fox");

    await clickBellButton(cdp);
    await clickChar(cdp, "rabbit");
    s = await snap(cdp);
    if ((s.inventory?.calm_bell ?? -1) !== stock0 - 1 || !(s.calm || []).includes("rabbit")) {
      throw new Error("calm hit fail: " + JSON.stringify(s));
    }
    await screenshot(cdp, path.join(EVID, "shots/08_c03_hit.png"));
    results.push({ step: "calm_hit_rabbit", pass: true });
    note("[PASS] calm hit rabbit");
  ...
  if (after.sha256 !== before.sha256 || after.bytes !== before.bytes) {
    throw new Error("dist changed during T-041 — forbidden");
  }

  const summary = {
    task: "T-041",
    kind: "candidates-playtest-ui-critical-regression",
    shippedClaim: false,
    input: "mouse-sim (CDP Input.dispatchMouseEvent)",
    deviceUntested: true,
    multitouchUntested: true,
    oneCommand: "node --experimental-websocket tests/evidence/scripts/t041-playtest-ui-critical-regression.mjs",
    results,
    pageErrors: cdp?.pageErrors || [],
    dist: after,
    expectedProductSha256: "19b8e65bbde285719ee43d74d83fcc60cdb3709ae7cc3ccc80d3f36d45d1bcf5",
    consolidates: ["T-031", "T-033", "T-035"],
  };
  ...
  writeFileSync(path.join(EVID, "command.txt"), summary.oneCommand + "\n");
