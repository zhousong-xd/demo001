/**
 * T-043 candidates playtest UI mouse-sim smoke (CDP Input.dispatchMouseEvent).
 * Sample UI short smoke: c04/c07/c10 place+undo (mouse-sim).
 * 真机 / 多指: 未测 — CDP ≠ 真机.
 *
 * Run from banquet-pilot/:
 *   node --experimental-websocket tests/evidence/scripts/t043-playtest-ui-sample-c04-c07-c10.mjs
 *
 * Evidence: evidence/t043-playtest-ui-sample-c04-c07-c10/
...
const EVID = path.join(ROOT, "evidence/t043-playtest-ui-sample-c04-c07-c10");
...
function buildChildEnv(extra = {}) {
  ...
  const env = {
    ...
    XDG_CONFIG_HOME: path.join(tmp, "banquet-t043-xdg-config"),
    XDG_CACHE_HOME: path.join(tmp, "banquet-t043-xdg-cache"),
    XDG_DATA_HOME: path.join(tmp, "banquet-t043-xdg-data"),
  };
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), "banquet-t043-chrome-"));
  ...
  if (after.sha256 !== before.sha256 || after.bytes !== before.bytes) {
    throw new Error("dist changed during T-043 — forbidden");
  }

  const summary = {
    task: "T-043",
    kind: "candidates-playtest-ui-sample-c04-c07-c10",
    shippedClaim: false,
  ...
  writeFileSync(path.join(EVID, "command.txt"),
    "cd banquet-pilot && node --experimental-websocket tests/evidence/scripts/t043-playtest-ui-sample-c04-c07-c10.mjs\n");
