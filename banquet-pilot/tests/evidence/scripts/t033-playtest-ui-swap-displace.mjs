/**
 * T-033 candidates playtest UI mouse-sim smoke (CDP Input.dispatchMouseEvent).
 * Covers: c02 seat swap via mouse; c01 waiting-guest displace via mouse.
 * 真机 / 多指: 未测 — CDP ≠ 真机.
 *
...
const EVID = path.join(ROOT, "evidence/t033-playtest-ui-swap-displace");
...
function buildChildEnv(extra = {}) {
  ...
  const env = {
    ...
    XDG_CONFIG_HOME: path.join(tmp, "banquet-t033-xdg-config"),
    XDG_CACHE_HOME: path.join(tmp, "banquet-t033-xdg-cache"),
    XDG_DATA_HOME: path.join(tmp, "banquet-t033-xdg-data"),
  };
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), "banquet-t033-chrome-"));
  ...
  try {
    ...
    // --- A: c02 swap ---
    await cdp.send("Page.navigate", { url: `http://127.0.0.1:${httpPort}/candidates/playtest/?c=c02` });
    await sleep(800);
    await waitPlaytest(cdp);
    let s = await snap(cdp);
    if (s.fileId !== "c02") throw new Error("boot not c02: " + JSON.stringify(s));
    await screenshot(cdp, path.join(EVID, "shots/01_c02_boot.png"));
    ...
    s = await snap(cdp);
    if (s.assignment?.fox !== "A1" || s.assignment?.rabbit !== "A2") {
      throw new Error("c02 pre-swap place failed: " + JSON.stringify(s));
    }
    await screenshot(cdp, path.join(EVID, "shots/02_c02_two_seated.png"));

    // swap: click seated fox, then click rabbit's seat A2 (or click rabbit char)
    await clickChar(cdp, "fox");
    await clickSeat(cdp, "A2");
    s = await snap(cdp);
    if (s.assignment?.fox !== "A2" || s.assignment?.rabbit !== "A1") {
      // fallback: click fox then click rabbit occupant center
      await cdp.eval(`(() => { /* reset via reload if needed */ })()`);
      throw new Error("c02 swap failed: " + JSON.stringify(s));
    }
    await screenshot(cdp, path.join(EVID, "shots/03_c02_swapped.png"));
    results.push({ step: "swap_c02", pass: true, snap: s });
    note("[PASS] mouse swap on c02: fox↔rabbit A1↔A2");

    // --- B: c01 displace ---
    await cdp.send("Page.navigate", { url: `http://127.0.0.1:${httpPort}/candidates/playtest/?c=c01` });
    await sleep(800);
    await waitPlaytest(cdp);
    s = await snap(cdp);
    if (s.fileId !== "c01") throw new Error("boot not c01: " + JSON.stringify(s));
    await screenshot(cdp, path.join(EVID, "shots/04_c01_boot.png"));

    await clickChar(cdp, "fox");
    await clickSeat(cdp, "A1");
    s = await snap(cdp);
    if (s.assignment?.fox !== "A1") throw new Error("c01 place fox failed: " + JSON.stringify(s));
    await screenshot(cdp, path.join(EVID, "shots/05_c01_fox_seated.png"));

    // waiting rabbit onto occupied A1 → displace fox
    await clickChar(cdp, "rabbit");
    await clickSeat(cdp, "A1");
    s = await snap(cdp);
    if (s.assignment?.rabbit !== "A1" || s.assignment?.fox !== null) {
      throw new Error("c01 displace failed: " + JSON.stringify(s));
    }
    await screenshot(cdp, path.join(EVID, "shots/06_c01_displaced.png"));
    results.push({ step: "displace_c01", pass: true, snap: s });
    note("[PASS] mouse displace on c01: rabbit takes A1, fox waiting");
  ...
  if (after.sha256 !== before.sha256 || after.bytes !== before.bytes) {
    throw new Error("dist changed during T-033 — forbidden");
  }

  const summary = {
    task: "T-033",
    kind: "candidates-playtest-ui-mouse-sim-swap-displace",
    shippedClaim: false,
  ...
  writeFileSync(path.join(EVID, "command.txt"),
    "cd banquet-pilot && node --experimental-websocket tests/evidence/scripts/t033-playtest-ui-swap-displace.mjs\n");
