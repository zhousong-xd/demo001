/**
 * T-037 candidates playtest UI mouse-sim smoke (CDP Input.dispatchMouseEvent).
 * Critical UI regression (one command): place/undo/switch, swap, displace, calm_bell.
 * 真机 / 多指: 未测 — CDP ≠ 真机.
 *
...
const EVID = path.join(ROOT, "evidence/t037-playtest-ui-critical-regression");
...
function buildChildEnv(extra = {}) {
  ...
  const env = {
    ...
    XDG_CONFIG_HOME: path.join(tmp, "banquet-t037-xdg-config"),
    XDG_CACHE_HOME: path.join(tmp, "banquet-t037-xdg-cache"),
    XDG_DATA_HOME: path.join(tmp, "banquet-t037-xdg-data"),
  };
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), "banquet-t037-chrome-"));
  ...
  if (after.sha256 !== before.sha256 || after.bytes !== before.bytes) {
    throw new Error("dist changed during T-037 — forbidden");
  }
  ...
  const summary = {
    ...
    oneCommand: "node --experimental-websocket tests/evidence/scripts/t037-playtest-ui-critical-regression.mjs",
    results,
