/**
 * T-031 candidates playtest UI mouse-sim smoke (CDP Input.dispatchMouseEvent).
 * Covers: load c01 + place waiting guest; undo restore; switch to c02 (not blank/wrong).
 * 真机 / 多指: 未测 — CDP ≠ 真机.
 *
...
const EVID = path.join(ROOT, "evidence/t031-playtest-ui-smoke");
...
function buildChildEnv(extra = {}) {
  ...
  const env = {
    ...
    XDG_CONFIG_HOME: path.join(tmp, "banquet-t031-xdg-config"),
    XDG_CACHE_HOME: path.join(tmp, "banquet-t031-xdg-cache"),
    XDG_DATA_HOME: path.join(tmp, "banquet-t031-xdg-data"),
  };
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), "banquet-t031-chrome-"));
  ...
  try {
    ...
    const url = `http://127.0.0.1:${httpPort}/candidates/playtest/?c=c01`;
    await cdp.send("Page.navigate", { url });
    await sleep(800);
    await waitPlaytest(cdp);
    await screenshot(cdp, path.join(EVID, "shots/01_c01_boot.png"));

    // 1) place fox → A1 via mouse
    let s0 = await snap(cdp);
    if (s0.fileId !== "c01" || !s0.hasStatus) throw new Error("boot not c01: " + JSON.stringify(s0));
    await clickChar(cdp, "fox");
    await clickSeat(cdp, "A1");
    let s1 = await snap(cdp);
    if (s1.assignment?.fox !== "A1" || s1.seated < 1) {
      throw new Error("place failed: " + JSON.stringify(s1));
    }
    if (s1.historyLen < 1) throw new Error("historyLen expected >=1 after place");
    await screenshot(cdp, path.join(EVID, "shots/02_c01_placed.png"));
    results.push({ step: "place_c01", pass: true, snap: s1 });
    note("[PASS] mouse place fox→A1 on c01");

    // 2) undo
    await clickUndo(cdp);
    let s2 = await snap(cdp);
    if (s2.assignment?.fox !== null && s2.assignment?.fox !== undefined) {
      // initial assignment is null for all
    }
    const seated2 = Object.values(s2.assignment || {}).filter(Boolean).length;
    if (seated2 !== 0 || s2.historyLen !== 0) {
      throw new Error("undo failed: " + JSON.stringify(s2));
    }
    await screenshot(cdp, path.join(EVID, "shots/03_c01_undo.png"));
    results.push({ step: "undo_c01", pass: true, snap: s2 });
    note("[PASS] mouse undo restores empty board");

    // 3) switch to c02
    await switchCandidateMouse(cdp, "c02");
    await waitPlaytest(cdp);
    let s3 = await snap(cdp);
    if (s3.fileId !== "c02" || s3.levelId !== "C02" || !s3.hasStatus) {
      throw new Error("switch c02 failed: " + JSON.stringify(s3));
    }
    if (!String(s3.url).includes("c=c02")) {
      throw new Error("url not updated to c02: " + s3.url);
    }
    await screenshot(cdp, path.join(EVID, "shots/04_c02_switched.png"));
    results.push({ step: "switch_c02", pass: true, snap: s3 });
    note("[PASS] switch c01→c02 not blank; url ?c=c02");
  ...
  if (after.sha256 !== before.sha256 || after.bytes !== before.bytes) {
    throw new Error("dist changed during T-031 — forbidden");
  }

  const summary = {
    task: "T-031",
    kind: "candidates-playtest-ui-mouse-sim",
    shippedClaim: false,
    input: "mouse-sim (CDP Input.dispatchMouseEvent) + select focus mouse then change",
    deviceUntested: true,
  ...
  writeFileSync(path.join(EVID, "command.txt"),
    "cd banquet-pilot && node --experimental-websocket tests/evidence/scripts/t031-playtest-ui-smoke.mjs\n");
