/**
 * T-050 candidates playtest UI mouse-sim smoke (CDP Input.dispatchMouseEvent).
 * Sample UI short smoke: c06/c09/c12 place+undo (mouse-sim).
 * 真机 / 多指: 未测 — CDP ≠ 真机.
 *
 * Run from banquet-pilot/:
 *   node --experimental-websocket tests/evidence/scripts/t050-playtest-ui-sample-c06-c09-c12.mjs
 *
 * Evidence: evidence/t050-playtest-ui-sample-c06-c09-c12/
...
const EVID = path.join(ROOT, "evidence/t050-playtest-ui-sample-c06-c09-c12");
...
function buildChildEnv(extra = {}) {
  ...
  const env = {
    ...
    XDG_CONFIG_HOME: path.join(tmp, "banquet-t050-xdg-config"),
    XDG_CACHE_HOME: path.join(tmp, "banquet-t050-xdg-cache"),
    XDG_DATA_HOME: path.join(tmp, "banquet-t050-xdg-data"),
  };
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), "banquet-t050-chrome-"));
  ...
  if (after.sha256 !== before.sha256 || after.bytes !== before.bytes) {
    throw new Error("dist changed during T-050 — forbidden");
  }

  const summary = {
    task: "T-050",
    kind: "candidates-playtest-ui-sample-c06-c09-c12",
    shippedClaim: false,
  ...
  writeFileSync(path.join(EVID, "command.txt"),
    "cd banquet-pilot && node --experimental-websocket tests/evidence/scripts/t050-playtest-ui-sample-c06-c09-c12.mjs\n");
